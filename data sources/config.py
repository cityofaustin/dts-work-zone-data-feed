turp_query = """
    ...
    FROM
        ...
        SELECT
            ...
            MAX(
                CASE WHEN INFOCODE = 72101 THEN
                    INFOVALUE
                END) AS secondary_permit,
            MAX(
                CASE WHEN INFOCODE = 79490 THEN
                    INFOVALUE
        ...
        FROM
            FOLDERFREEFORM
        WHERE
            FREEFORMCODE in(1010, 1015)
            AND C02 in('Traffic Lane : Dimensions', 'Closure : Full Road', 'Closure : Alley', 'Closure : Sidewalk', 'Parking Lane : Dimensions', 'Open Cuts : Street')
            AND C03 = 'Yes') ff ON ff.FOLDERRSN = f.FOLDERRSN
    WHERE (f.FOLDERTYPE = 'RW'
        AND f.SUBCODE = 50500
        OR(f.FOLDERTYPE = 'EX')) -- Temporary use of ROW permits (TURPs)
